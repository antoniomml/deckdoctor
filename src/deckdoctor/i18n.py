...
_EN: dict[str, str] = {
    ...
    "cli.yes": "Apply the printed plan without asking",
    ...
    "ui.fix_none": "deckdoctor fix        nothing safe to apply",
    "ui.readonly": "Diagnose never changes anything. `deckdoctor fix` asks first.",
    "ui.wrote": "Wrote {path}",
    ...
    "fix.need_yes": "Nothing was changed. If the plan looks right, run it again with -y.",
    "fix.confirm": "Apply this plan? [y/N] ",
    "fix.declined": "Nothing was changed.",
    "fix.need_tty": "Not a terminal. Re-run with -y to apply.",
    ...
    "report.posture": "- Posture: diagnose is read-only; `deckdoctor fix` (TTY confirm or `-y`) is the only mutating command.",
    ...
    "diag.incomplete_rate.summary": (
        "PluginLoader is missing and GitHub API remaining is 0. "
        "The installer likely could not download it — common on CGNAT."
    ),
    ...
    "diag.unit_429.summary": (
        "plugin_loader.service contains GitHub's rate-limit HTML. "
    ...
    "diag.incomplete.summary": "The homebrew tree exists but PluginLoader was never downloaded.",
    ...
    "diag.port.summary": (
        "PluginLoader is present but port 8080 is not Steam's CEF debugger. "
        "Another process is blocking Decky injection."
    ),
    ...
    "diag.steam_beta.summary": (
        "Decky's backend looks healthy and port 8080 looks like Steam CEF, "
        "but the Steam client is on Beta. A missing QAM tab is more likely the client than a broken install."
    ),
    "diag.steam_beta.rec": (
    ...
    "diag.autoflatpaks.summary": (
        "AutoFlatpaks is installed and `flatpak` works, but `flatpak remote-ls` failed. "
        "The plugin cannot list packages until Flatpak can."
    ),
    ...
    "diag.overlay.summary": (
        "atomupd/rauc configs exist in the SteamOS /etc overlay, and the updater failed a read-only check. "
        "Those overlay copies may be hiding the image configs the updater expects."
    ),
    "diag.overlay.rec": (
...
_ES: dict[str, str] = {
    ...
    "cli.yes": "Aplicar el plan sin preguntar",
    ...
    "ui.fix_none": "deckdoctor fix        nada seguro que aplicar",
    "ui.readonly": "Diagnosticar no cambia nada. `deckdoctor fix` pregunta antes.",
    ...
    "fix.need_yes": "No se ha cambiado nada. Si te encaja el plan, vuelve a ejecutarlo con -y.",
    "fix.confirm": "¿Aplicar este plan? [y/N] ",
    "fix.declined": "No se ha cambiado nada.",
    "fix.need_tty": "No es un terminal. Vuelve a ejecutarlo con -y para aplicar.",
    ...
    "report.posture": "- Postura: diagnose es solo lectura; `deckdoctor fix` (confirmación en TTY o `-y`) es el único comando que muta.",
    ...
    "diag.incomplete_rate.summary": (
        "Falta PluginLoader y la cuota de GitHub restante es 0. "
        "El instalador probablemente no pudo descargarlo (habitual en CGNAT)."
    ),
    "diag.incomplete_rate.rec": (
    ...
    "diag.unit_429.summary": (
        "plugin_loader.service contiene el HTML de límite de GitHub. "
    ...
    "diag.incomplete.summary": "Existe el árbol homebrew pero PluginLoader nunca se descargó.",
    ...
    "diag.port.summary": (
        "PluginLoader está presente pero el puerto 8080 no es el depurador CEF de Steam. "
        "Otro proceso bloquea la inyección de Decky."
    ),
    ...
    "diag.steam_beta.summary": (
        "El backend de Decky se ve sano y el puerto 8080 parece CEF de Steam, "
        "pero el cliente Steam está en Beta. Si falta la pestaña del QAM, es más probable el cliente que una instalación rota."
    ),
    "diag.steam_beta.rec": (
    ...
    "diag.autoflatpaks.summary": (
        "AutoFlatpaks está instalado y `flatpak` funciona, pero `flatpak remote-ls` falló. "
        "El plugin no puede listar paquetes hasta que Flatpak pueda."
    ),
    ...
    "diag.overlay.summary": (
        "Hay configs atomupd/rauc en el overlay /etc de SteamOS, y el actualizador falló una consulta de solo lectura. "
        "Esas copias del overlay pueden estar ocultando las configs de la imagen que el actualizador espera."
    ),
    "diag.overlay.rec": (
...
def detect_locale(explicit: str | None = None) -> Locale:
    ...
    raw = (os.environ.get("DECKDOCTOR_LANG") or "").lower().replace("-", "_")
